  ...
  <div>
    ...
    <Row style="margin-top:2rem">
      <iCol
        ...
        :lg="{ span: 14, offset: 5 }"
      >
        <Page :total=count :current.sync=pageCount size="small" show-elevator simple @on-change="changePage"
              style="float:left"/>
        <Select
          size="small"
      ...
      >
        ...
        >
          ...
          <div id="wordcount">
            ...
            字数：{{item.article.article_wordCount}}
            <Divider type="vertical"/>
            <Icon type="ios-happy-outline"/>
            性向：{{item.article.article_category}}
          </div>
          <div id="relationship">
    ...
    methods: {
        ...
        this.$axios.post(
          ...
          if (res.status === 200) {
            if (res.data.msg === 'fandom followed!') {
              ...
              if (index > -1) {
                this.followedFandom.splice(index, 1);
              }
            }
            this.$Spin.hide();
          }
      ...
      fetchSearchResult(offset, limit) {
        ...
        if (!this.easySearch) { //复杂搜索
          ...
          if (this.fandomList.length != 0) {
            data.fandom = this.fandomList
          }
          if (this.relationshipList.length != 0) {
            data.relationship = this.relationshipList
          }
          data.limit = limit;
          data.offset = offset;
